...
void *projeto(void *args){

    // identificador da thread 
    int id = *(int *) args;

    // calculando o tamanho de cada bloco
    int bloco = rows / num_threads;

    // calculando o inicio e o fim de cada bloco considerando a fronteira HALF_LENGTH
    int begin = id * bloco + HALF_LENGTH;
    int end = begin + bloco;

    // Definindo o final da última thread
    if( id == num_threads-1 )
        end = rows - HALF_LENGTH;
    ...
    // criando o array p_threads
    pthread_t threads[num_threads];

    // guardando o identificador das threads
    ...
    // criando as threads com o create
    	for(int i = 0; i < num_threads; i++){
        	t_id[i] = i;
        	pthread_create(&threads[i], NULL, projeto, (void *) &t_id[i]);
    	}

    // fazendo o join da sincronização
    	for(int i = 0; i < num_threads; i++)
